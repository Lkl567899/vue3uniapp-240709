...
import {
  getMemberAddressIdAPI,
  postMemberAddressAPI,
  putMemberAddressIdAPI,
...
// 动态设置标题
uni.setNavigationBarTitle({ title: query.id ? '编辑地址' : '添加地址' })
// 表单数据
const form = ref({
  receiver: '', // 收货人
  contact: '', // 联系方式
...
// 修改默认地址
const switchChange: UniHelper.SwitchOnChange = (ev) => {
...
const submit = async () => {
  ...
  if (query.id) {
    await putMemberAddressIdAPI(query.id, form.value)
  } else {
    // 新增收货地址
