...
// 删除订单
const onOrderDel = async () => {
  await deleteMemberOrderAPI({ ids: [query.id] })
  uni.redirectTo({ url: '/pagesOrder/list/list' })
}
// 取消订单
const onDescription = async () => {
  const res = await putMemberOrderByIdAPI(query.id, { cancelReason: reason.value })
  console.log(res)
  order.value!.orderState = OrderState.YiQuXiao
  // 调用
  getMemberOrderByIdData()
    ...
    <template v-if="order">
      ...
      <view class="overview" :style="{ paddingTop: safeAreaInsets!.top + 20 + 'px' }">
        ...
        <template v-if="order?.orderState === OrderState.DaiFuKuan">
          ...
          <view class="button" @tap="vxPay">去支付</view>
        </template>
        <!-- 其他订单状态:展示再次购买按钮 -->
        <template v-else>
          <!-- 订单状态文字 -->
      ...
      <view class="detail">
        ...
        <view class="row">
          ...
          <view class="item">下单时间: {{ order.createTime }}0</view>
        </view>
      </view>
      ...
      <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
        <!-- 待付款状态:展示支付按钮 -->
        <template v-if="order.orderState === OrderState.DaiFuKuan">
          <view class="button primary" @tap="vxPay"> 去支付 </view>
  ...
  <uni-popup ref="popup" type="bottom" background-color="#fff">
    <view class="popup-root">
      ...
      <view class="footer">
        ...
        <view class="button primary" @tap="onDescription">确认</view>
      </view>
    </view>
  </uni-popup>
</template>
